...
/**
 * Sorts an array in natural ascending order, placing zero at the end if `zeroIsLast` is `true`.
 */
...
recurrify.day = recurrify.D = {
   ...
   * Use `0` to specify the last day of the month.
   *
  ...
  extent: function (d) {
    ...
    const month = recurrify.M.val(d)

    let max = recurrify.DAYS_IN_MONTH[month - 1]
   ...
   * Returns the first day of the next month if `val` is greater than the number of days in the following month.
   *
  ...
  next: function (d, val) {
    ...
    const month = recurrify.date.nextRollover(d, val, recurrify.D, recurrify.M)

    const DMax = recurrify.D.extent(month)[1]
  ...
  /**
   * Returns the end of the previous instance of the day value indicated.
   * Returns the last day in the previous month if val is greater than the number of days in the previous month.
   *
   * @param {Date} d: The starting date.
   * @param {int} val: The desired value, must be within extent.
   */
  prev: function (d, val) {
    const month = recurrify.date.prevRollover(d, val, recurrify.D, recurrify.M)
...
/**
 * Definition for a day of week count constraint type.
 * This constraint is used to specify recurrences like "2nd Tuesday of every month".
 */
recurrify.dayOfWeekCount = recurrify.dc = {
  ...
  next: function (d, val) {
    ...
    let month = recurrify.date.nextRollover(d, val, recurrify.dc, recurrify.M)

    const dcMax = recurrify.dc.extent(month)[1]

    val = val > dcMax ? 1 : val

    const next = recurrify.date.next(
      recurrify.Y.val(month),
      recurrify.M.val(month),
   ...
   */
  prev: function (d, val) {
    const month = recurrify.date.prevRollover(d, val, recurrify.dc, recurrify.M)
...
recurrify.dayOfWeek = recurrify.dw = recurrify.d = {
  ...
  /**
   * The minimum and maximum valid day of week values.
   * Unlike the standard `Date` object, recurrify day of week goes from `1` to `7`.
   */
  extent: function () {
    return [1, 7]
  },
...
recurrify.dayOfYear = recurrify.dy = {
  ...
  next: function (d, val) {
    ...
    const year = recurrify.date.nextRollover(d, val, recurrify.dy, recurrify.Y)

    const dyMax = recurrify.dy.extent(year)[1]
   ...
   */
  prev: function (d, val) {
    const year = recurrify.date.prevRollover(d, val, recurrify.dy, recurrify.Y)
...
recurrify.minute = recurrify.m = {
   ...
   */
  next: function (d, val) {
    const m = recurrify.m.val(d)

    const s = recurrify.s.val(d)

    const inc = val > 59 ? 60 - m : (val <= m ? (60 - m) + val : val - m)

    let next = new Date(d.getTime() + (inc * recurrify.MIN) - (s * recurrify.SEC))
...
recurrify.month = recurrify.M = {
   ...
   * Unlike the native date object, month values in recurrify are `1`-based.
   */
  extent: function () {
    return [1, 12]
  },
...
recurrify.second = recurrify.s = {
  ...
  next: function (d, val) {
    ...
    let next = new Date(d.getTime() + (inc * recurrify.SEC))
...
recurrify.weekOfMonth = recurrify.wm = {
  ...
  next: function (d, val) {
    ...
    const month = recurrify.date.nextRollover(d, val, recurrify.wm, recurrify.M)

    const wmMax = recurrify.wm.extent(month)[1]
   ...
   */
  prev: function (d, val) {
    const month = recurrify.date.prevRollover(d, val, recurrify.wm, recurrify.M)
...
recurrify.weekOfYear = recurrify.wy = {
  ...
  val: function (d) {
    ...
    const wThur = recurrify.dw.next(recurrify.wy.start(d), 5)

    const YThur = recurrify.dw.next(recurrify.Y.prev(wThur, recurrify.Y.val(wThur) - 1), 5)
  ...
  extent: function (d) {
    ...
    const year = recurrify.dw.next(recurrify.wy.start(d), 5)

    const dwFirst = recurrify.dw.val(recurrify.Y.start(year))

    const dwLast = recurrify.dw.val(recurrify.Y.end(year))
  ...
  next: function (d, val) {
    ...
    const wyThur = recurrify.dw.next(recurrify.wy.start(d), 5)

    let year = recurrify.date.nextRollover(wyThur, val, recurrify.wy, recurrify.Y)
    ...
    const wyMax = recurrify.wy.extent(year)[1]

    const wyStart = recurrify.wy.start(year)
  ...
  prev: function (d, val) {
    ...
    let year = recurrify.date.prevRollover(wyThur, val, recurrify.wy, recurrify.Y)
    ...
    const wyEnd = recurrify.wy.end(year)
...
/**
 * Compiles a single recurrence definition into a form from which instances can be efficiently calculated from.
 */
recurrify.compile = function (schedDef) {
  ...
  for (const key in schedDef) {
    ...
    const constraint = mod ? recurrify.modifier[mod](recurrify[name], vals) : recurrify[name]
  ...
  /**
   * Returns a function to use when comparing two dates.
   * Encapsulates the difference between searching for instances forward and backwards so that the same code can be completely reused for both directions.
   *
  ...
  return {
    /**
     * Calculates the start of the next valid time slot of a particular recurrence that occurs on or after the specified start time.
     *
    ...
    start: function (dir, startDate) {
      ...
      const nextVal = recurrify.array[dir]
      ...
      while (maxAttempts-- && !done && next) {
        ...
        for (let i = 0; i < constraintsLen; i++) {
          ...
          const newVal = nextVal(curVal, constraints[i].vals, extent)
     ...
     */
      ...
      const nextVal = recurrify.array[dir + 'Invalid']
...
/**
 * Returns an object to calculate future or previous time slots of the specified recurrence.
 */
recurrify.recurrence = function (sched) {
  ...
  const recurrencesLen = sched.recurrences.length
  ...
  for (let i = 0; i < recurrencesLen; i++) {
    for (const key in sched.recurrences[i]) {
      if (sched.recurrences[i][key].length === 0) {
        delete sched.recurrences[i][key]
      }
    }
  }
  for (let i = 0; i < exceptionsLen; i++) {
    for (const key in sched.exceptions[i]) {
      if (sched.exceptions[i][key].length === 0) {
        delete sched.exceptions[i][key]
      }
    }
  }
  ...
  /**
   * Calculates count number of instances or ranges for the current recurrence, optionally between the specified `startDate` and `endDate`.
   *
   * @param {String} dir: The direction to use, either `next` or `prev`.
   ...
   * @param {Boolean} isRange: `true` to return ranges, `false` to return instances.
   */
  ...
  /**
   * Updates the set of cached start dates to the next valid start dates.
   * Only recurrences where the current start date is less than or equal to the specified `startDate` need to be updated.
   *
   * @param {String} dir: The direction to use, either `next` or `prev`.
   * @param {Array} schedArr: The set of compiled recurrences to use.
   * @param {Array} startsArr: The set of cached start dates for the recurrences.
  ...
  /**
   * Updates the set of cached ranges to the next valid ranges.
   * Only recurrences where the current start date is less than or equal to the specified `startDate` need to be updated.
   *
   * @param {String} dir: The direction to use, either `next` or `prev`.
   * @param {Array} schedArr: The set of compiled recurrences to use.
  ...
  /**
   * Calculates the end of the overlap between any exception recurrence and the specified start date.
  ...
  /**
   * Calculates the earliest start of an exception recurrence
   * This is the maximum end date of the recurrence.
   *
  ...
  /**
   * Calculates the next invalid date for a particular recurrences starting from the specified valid start date.
   *
   * @param {String} dir: The direction to use, either `next` or `prev`.
   * @param {Array} schedArr: The set of compiled recurrences to use.
   * @param {Array} startsArr: The set of cached start dates for the recurrences.
  ...
  /**
   * Returns the next value in an array using the function passed in as compare to do the comparison.
   * Skips over `null` or `undefined` values.
   *
  ...
  return {
    ...
    /**
     * Returns `true` if d is a valid time slot of the current recurrence.
     *
    ...
    /**
     * Finds the next valid instance or instances of the current recurrence, optionally between a specified start and end date.
     * `startDate` is `Date.now()` by default, end date is unspecified.
    ...
    /**
     * Finds the previous valid instance or instances of the current recurrence, optionally between a specified start and end date.
     * `startDate` is `Date.now()` by default, end date is unspecified.
     * `startDate` must be smaller than `endDate`.
     *
     * @param {Integer} count: The number of instances to return.
    ...
    /**
     * Finds the next valid range or ranges of the current recurrence, optionally between a specified start and end date.
     * `startDate` is `Date.now()` by default, end date is unspecified.
    ...
    /**
     * Finds the previous valid range or ranges of the current recurrence, optionally between a specified start and end date.
     * `startDate` is `Date.now()` by default, end date is unspecified.
     * `startDate` must be greater than `endDate`.
     *
     * @param {Integer} count: The number of ranges to return.
...
/**
 * Configures helper functions to switch between using local time and UTC.
 * `recurrify` uses UTC time by default.
 */
...
/**
 * Creates a new `Date` object defaulted to the first second after the specified values.
 */
/**
 * Builds and returns a new Date using the specified values.
 * Date returned is either using Local time or UTC based on isLocal.
 *
...
/**
 * Determines if a value will cause a particular constraint to rollover to the next largest time period.
 * Used primarily when a constraint has a variable extent.
 */
...
/**
 * Creates a new Date object defaulted to the last second after the specified values.
 */
/**
 * Builds and returns a new Date using the specified values.
 * Date returned is either using Local time or UTC based on `isLocal`.
...
/**
 * Determines if a value will cause a particular constraint to rollover to the previous largest time period.
 * Used primarily when a constraint has a variable extent.
 */
